...
import { type Keys } from '@internxt/sdk'
import { type UserSettings } from '@internxt/sdk/dist/shared/types/userSettings'
import * as openpgp from 'openpgp'
import { generateNewKeys, getOpenpgp } from './pgp.service'
import {
...
export class KeysService {
  ...
  public async getKeys(password: string): Promise<Keys> {
    ...
    const encPrivateKey = aes.encrypt(
      privateKeyArmored,
      password,
      this.getAesInit(),
    )
    const encPrivateKyberKey = aes.encrypt(
      privateKyberKeyBase64,
      password,
      this.getAesInit(),
    )
  ...
  public parseAndDecryptUserKeys(
    ...
    if (user.keys?.kyber?.privateKey) {
      privateKyberKey = this.decryptPrivateKey(
   ...
   * @async
   */
  public assertPrivateKeyIsValid = async (
    privateKey: string,
    password: string,
  ): Promise<void> => {
  ...
  public encryptPrivateKey = (privateKey: string, password: string): string => {
    return aes.encrypt(privateKey, password, this.getAesInit())
  }
  ...
  public decryptPrivateKey = (privateKey: string, password: string): string => {
    if (!privateKey || privateKey.length <= this.MINIMAL_ENCRYPTED_KEY_LEN)
      return ''
    else {
      try {
   ...
   * @async
   **/
  public assertValidateKeys = async (
    privateKey: string,
    publicKey: string,
  ): Promise<void> => {

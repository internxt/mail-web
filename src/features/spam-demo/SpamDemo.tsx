...
const tokenizer: object = await loadTokenizer();
...
async function loadVocab(path: string) {
  ...
  for (let i = 0; i < rows.length; i++) {
    const key = rows[i][1];
    const key_int = BigInt(Number(key.replace(',', '').trim()));
    const value: string = rows[i][0];
    const value1 = value.replace('"', '').trim();
...
function provide_explanation(
  ...
  for (let i = 0; i < LENTOKENS; i++) {
    strexp = strexp.concat(
      `<span style="color:${color(relevancies[i])}"> <b> ${finalVocab.get(outputTokens[i])} </b> </s> </span> `,
    );
  }
  container(strexp);
}
...
const SpamDemo = ({ folder }: MailViewProps) => {
  ...
  const [mail, setMail] = useState('Content of the message...');
  ...
  const evalDemo = () => {
    spamOrHam(mail, setText, setExpl);
  };
